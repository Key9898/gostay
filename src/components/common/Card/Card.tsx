...
export default function Card({
  ...
  return (
    <div
      onClick={onClick}
      className={cn('warm-card group overflow-hidden', onClick && 'cursor-pointer', className)}
    >
      {image && (
        <figure className="relative h-44 overflow-hidden bg-base-200 sm:h-56">
          <img
            src={image}
      ...
      <div className="p-5">
        {title && (
          <h3 className="font-serif text-lg font-semibold leading-snug text-base-content transition-colors group-hover:text-primary sm:text-xl">
            {title}
          </h3>
        )}
        {children}
